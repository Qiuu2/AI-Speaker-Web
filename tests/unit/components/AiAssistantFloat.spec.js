...
describe('AiAssistantFloat diagnostics', () => {
  ...
  it('shows warning notifications returned by assistant chat', async() => {
    const wrapper = createWrapper()
    ...
    await wrapper.vm.submitAssistantText('播放国歌')
    await flushPromises()
  ...
  it('fills playback media duration template with seconds and volume text', () => {
    ...
    const variant = wrapper.vm.resolveManualItem(item)

    expect(variant.slotMap['终端/区域'].type).toBe('target')

    wrapper.vm.setSlotValue(item, '终端/区域', '操场')
    wrapper.vm.setSlotValue(item, '媒体', '电铃声.mp3')
    wrapper.vm.setSlotValue(item, '秒数', '10')
    wrapper.vm.setSlotValue(item, '数值', '20')
    wrapper.vm.fillFromTemplate(item)

    expect(wrapper.vm.command).toBe('给操场播放电铃声.mp3，播放10秒，音量20')
  })
  ...
  it('fills playback media loop template with count and volume text', () => {
    ...
    wrapper.vm.setSlotValue(item, '终端/区域', '右一终端')
    wrapper.vm.setSlotValue(item, '媒体', '上课铃')
    wrapper.vm.setSlotValue(item, '次数', '3')
    wrapper.vm.setSlotValue(item, '数值', '40')
    wrapper.vm.fillFromTemplate(item)

    expect(wrapper.vm.command).toBe('给右一终端播放上课铃，播放3次，音量40')
  })

  it('uses broadcast task options for immediate task playback template', () => {
    const wrapper = createWrapper()
    const playbackModule = wrapper.vm.manualModules.find((module) => module.id === 'playback')
    const item = playbackModule.items.find((entry) => entry.id === 'playback-task')

    const segments = wrapper.vm.parseTemplate(item)
    const slot = segments.find((seg) => seg.type === 'slot')

    expect(slot.key).toBe('文件广播')
    expect(slot.slotType).toBe('broadcastTask')

    wrapper.vm.setSlotValue(item, '文件广播', '大课间')
    wrapper.vm.fillFromTemplate(item)

    expect(wrapper.vm.command).toBe('播放大课间任务')
  })

  it('prefers broadcast task names over media names for broadcast task options', () => {
    const wrapper = createWrapper()

    expect(wrapper.vm.pickTaskName({
      taskname: 'morning-broadcast',
      audio: 'bell.mp3',
      medianame: 'bell.mp3'
    })).toBe('morning-broadcast')
  })
  ...
  it('invalidates schedule task caches after schedule task changes', () => {
    const wrapper = createWrapper()
    ...
    wrapper.setData({
      ...
      slotOptions: {
        schedule: [{ label: 'schedule-a', value: 'schedule-a' }],
        broadcastTask: [{ label: 'task-a', value: 'task-a' }],
        terminal: [{ label: 'terminal-a', value: 'terminal-a' }]
      }
    })
    ...
    wrapper.vm.handleAssistantRefresh({
      ...
      action_log: [{ action: 'move_schedule' }]
    })

    expect(wrapper.vm.slotOptions.schedule).toEqual([])
    expect(wrapper.vm.slotOptions.broadcastTask).toEqual([])
    expect(wrapper.vm.slotOptions.terminal).toEqual([{ label: 'terminal-a', value: 'terminal-a' }])
    expect(refreshSpy).toHaveBeenCalledWith({
      force: true,
      types: ['schedule', 'broadcastTask']
    })
