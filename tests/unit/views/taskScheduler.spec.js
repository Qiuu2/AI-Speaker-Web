...
jest.mock('@/utils/schedulerStorage', () => ({
  // T51 ②: loadBroadcasts now consults the real substantive-equality check;
  // use the actual implementation so a name-differing draft is correctly judged
  // NOT substantively equal (kept), while a derived-only diff would auto-clear.
  areBroadcastRowsSubstantivelyEqual: jest.requireActual('@/utils/schedulerStorage').areBroadcastRowsSubstantivelyEqual,
...
describe('TaskSchedulerPage', () => {
  ...
  it('T15: buildAllTaskRow carries the selected weekdays so the backend T61 bridge can derive execmode', () => {
    // PO hh bug: the add payload sent execmode=0 + NO weekdays, so the
    // schedule's周期 never persisted (build_remote_taskinfo_payload had nothing
    // to derive execmode from) and the optimistic row forked from the canonical
    // backfill (weekdays=[] from execmode=0) → phantom draft. The payload must
    // carry weekdays. Reverse-truth: dropping the weekdays line makes
    // result.weekdays undefined here and the backend would persist execmode=0.
    const { methods } = TaskSchedulerPage
    const ctx = {
      normalizeRealTaskId: methods.normalizeRealTaskId,
      toTime: (value) => value,
    ...
    const withWeekdays = methods.buildAllTaskRow.call(ctx, {
      id: 'draft-broadcast-2', taskid: 'draft-broadcast-2', name: 'hh', audio: 'a.mp3',
      durationMode: 'loop', loop: 1, weekdays: ['周一', '周二', '周三', '周日']
    }, 2, '2026-03-23')
    // execmode stays 0 in the payload; the backend bridge encodes from weekdays.
    expect(withWeekdays.execmode).toBe(0)
    expect(withWeekdays.weekdays).toEqual(['周一', '周二', '周三', '周日'])

    // manual play (no weekdays) → empty array, bridge leaves execmode=0.
    const noWeekdays = methods.buildAllTaskRow.call(ctx, {
      id: 'draft-broadcast-3', taskid: 'draft-broadcast-3', name: 'm', audio: 'a.mp3',
      durationMode: 'loop', loop: 1
    }, 2, '2026-03-23')
    expect(noWeekdays.weekdays).toEqual([])
  })
  ...
  it('keeps once edit drawer open when saving fails', async() => {
    const { methods } = TaskSchedulerPage
    updateOnceOverrideTask.mockRejectedValueOnce({
      response: {
        data: {
  ...
  it('reloads server state when pending plan save fails', async() => {
    const { methods } = TaskSchedulerPage
    updateBroadcastSchedules.mockRejectedValueOnce({
      response: {
        data: {
          detail: 'override cleanup failed'
        }
      }
    })
  ...
  it('keeps once drawer state when deleting a once panel item fails', async() => {
    const { methods } = TaskSchedulerPage
    deleteOnceOverrideTask.mockRejectedValueOnce({
      response: {
        data: {
  ...
  it('undoes a whole once override batch from the drawer group header', async() => {
    const { methods } = TaskSchedulerPage
    undoOnceOverride.mockResolvedValueOnce({ status: 'ok' })
    const loadModules = jest.fn()
    const closeOnceChangesDrawer = jest.fn()
    const message = { success: jest.fn(), warning: jest.fn(), error: jest.fn() }

    await methods.undoOncePanelGroup.call({
      confirmDelete: jest.fn().mockResolvedValue(true),
      loadModules,
      onceDrawerGroups: [{ items: [{ onceTaskId: '93001' }] }],
      closeOnceChangesDrawer,
      ...
      overrideId: 'override-1',
      canUndo: true,
      items: [{ onceTaskId: '93001' }, { onceTaskId: '93002' }]
    })

    expect(undoOnceOverride).toHaveBeenCalledTimes(1)
    expect(undoOnceOverride).toHaveBeenCalledWith('override-1')
    expect(loadModules).toHaveBeenCalled()
    expect(message.success).toHaveBeenCalledWith('已撤销本批一次性任务')
  })
  it('refreshes once drawer data after a partial batch undo failure', async() => {
    const { methods } = TaskSchedulerPage
    undoOnceOverride.mockRejectedValueOnce({
      response: {
        data: {
          detail: {
            status: 'partial_failed',
            message: '部分撤销失败，已刷新剩余临时任务'
          }
        }
      }
    })
    ...
    await methods.undoOncePanelGroup.call({
      confirmDelete: jest.fn().mockResolvedValue(true),
      loadModules,
      onceDrawerGroups: [{ items: [{ onceTaskId: '93002' }] }],
      closeOnceChangesDrawer,
      $message: message
    }, {
      overrideId: 'override-1',
    ...
    expect(undoOnceOverride).toHaveBeenCalledWith('override-1')
    expect(loadModules).toHaveBeenCalled()
    expect(message.warning).toHaveBeenCalledWith('部分撤销失败，已刷新剩余临时任务')
