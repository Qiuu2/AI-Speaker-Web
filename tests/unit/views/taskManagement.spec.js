...
describe('TaskManagementPage', () => {
  ...
  describe('periodic broadcast merge (T60 阶段1)', () => {
    const { methods } = TaskManagementPage

    it('isPeriodicBroadcast: keeps execmode>0 or non-empty weekdays, drops one-off', () => {
      expect(methods.isPeriodicBroadcast({ execmode: 62 })).toBe(true)
      expect(methods.isPeriodicBroadcast({ weekdays: ['周一'] })).toBe(true)
      expect(methods.isPeriodicBroadcast({ execmode: 0, weekdays: [] })).toBe(false)
      expect(methods.isPeriodicBroadcast({ execmode: 0 })).toBe(false)
      expect(methods.isPeriodicBroadcast(null)).toBe(false)
    })
    ...
    function mapCtx() {
      return {
        taskDisplayName: methods.taskDisplayName,
        taskSourceName: methods.taskSourceName,
        // formatLocation pulls in normalizeZoneLabel etc.; stub at that seam
        // since the broadcast-mapping shape (not location formatting) is the SUT.
        formatLocation: (loc) => (Array.isArray(loc) && loc.length ? '终端' : ''),
        toTime: methods.toTime,
        toDurationSeconds: methods.toDurationSeconds,
        taskKey: methods.taskKey
      }
    ...
    it('mapBroadcastTask: empty scheduleName + 文件广播 source label/badge', () => {
      const row = {
        taskid: '74206', name: '大课间', audio: '大课间.mp3',
        starttime: '08:00:00', timelength: '300', timelengthtype: 1,
        weekdays: ['周一'], volume: 70, location: [['操场', '右一终端']], enablestate: 1
      }
      const mapped = methods.mapBroadcastTask.call(mapCtx(), row, 0, '2026-06-15')
      expect(mapped.scheduleName).toBe('')
      expect(mapped.kind).toBe('broadcast')
      expect(mapped.broadcastSource).toBe(true)
      expect(mapped.badge).toBe('文件广播')
      expect(mapped.taskId).toBe('74206')
      expect(mapped.displayName).toBe('大课间')
      expect(mapped.time).toBe('08:00:00')
    })

    // critic-daily1 f1: broadcast rows are display-only — isTaskReadonly must be
    // true so no toggle/edit/delete buttons render and they are not bulk-selectable.
    it('isTaskReadonly: broadcast rows are read-only, schedule tasks are not', () => {
      expect(methods.isTaskReadonly({ broadcastSource: true })).toBe(true)
      // reverse-truth: a normal schedule task (no broadcastSource / no once flag)
      // stays editable — removing the broadcastSource gate would make this branch
      // identical and the broadcast assertion above would fail.
      expect(methods.isTaskReadonly({ scheduleName: '夏季作息', taskId: '1' })).toBe(false)
      // once-ephemeral rows remain read-only (pre-existing behavior preserved)
      expect(methods.isTaskReadonly({ isOnceEphemeral: true })).toBe(true)
    })

    function timelineCtx(schedules, broadcasts, weekdayLabel) {
      return {
        schedules,
        broadcasts,
        // stub the date→weekday seam (date arithmetic isn't the SUT here)
        weekdayLabelShanghai: () => weekdayLabel,
        isScheduleEnabled: () => true,
        isTaskActiveOnDate: methods.isTaskActiveOnDate,
        weekdaysFromExecmode: methods.weekdaysFromExecmode,
        normalizeDate: methods.normalizeDate,
        mapScheduleTask: methods.mapScheduleTask,
        mapBroadcastTask: methods.mapBroadcastTask,
        taskDisplayName: methods.taskDisplayName,
        taskSourceName: methods.taskSourceName,
        // formatLocation pulls in normalizeZoneLabel etc.; stub at that seam
        // since the broadcast-mapping shape (not location formatting) is the SUT.
    ...
    it('buildBaseTasksForDate: a periodic broadcast appears on a matching weekday', () => {
      // 2026-06-15 is a Monday → 周一
      const broadcasts = [
        { taskid: '74206', name: '大课间', audio: '大课间.mp3', starttime: '08:00:00', weekdays: ['周一'], enablestate: 1 }
      ]
      const ctx = timelineCtx([], broadcasts, '周一')
      const tasks = methods.buildBaseTasksForDate.call(ctx, '2026-06-15')
      const broadcastTasks = tasks.filter((t) => t.broadcastSource)
      expect(broadcastTasks).toHaveLength(1)
      expect(broadcastTasks[0].taskId).toBe('74206')
    })

    it('buildBaseTasksForDate: a periodic broadcast is absent on a non-matching weekday', () => {
      // 2026-06-16 is a Tuesday → 周二, broadcast only fires 周一
      const broadcasts = [
        { taskid: '74206', name: '大课间', audio: '大课间.mp3', starttime: '08:00:00', weekdays: ['周一'], enablestate: 1 }
      ]
      const ctx = timelineCtx([], broadcasts, '周二')
      const tasks = methods.buildBaseTasksForDate.call(ctx, '2026-06-16')
      expect(tasks.filter((t) => t.broadcastSource)).toHaveLength(0)
    })
  })
})
