...
describe('onceTaskSpecs', () => {
  ...
  it('groups once panel items by date and action after filtering', () => {
    ...
    expect(groups[0].overrideId).toBe('override-1')
    expect(groups[0].canUndo).toBe(true)
  ...
  it('keeps separate undo batches for different overrides on the same date and action', () => {
    const overrides = ['override-a', 'override-b'].map((id, index) => ({
      id,
      mode: 'once',
      action: 'migrate',
      schedule_name: '夏季作息',
      ...
      remote_synced: true,
      cleanup_state: 'pending',
      created_at: `2026-04-20 0${index + 8}:00:00`,
      time_start: `2026-04-25 0${index + 7}:50:00`,
      new_time_start: `2026-04-26 0${index + 7}:50:00`,
      task_ids: [`8129${index + 1}`],
      once_task_specs: [{
        taskid: `9300${index + 1}`,
        taskname: `临时任务 ${index + 1}`,
        startdate: '2026-04-26',
        starttime: index === 0 ? '07:50:00' : '08:10:00',
        once_action: 'migrate'
      }]
    }))

    const groups = groupOncePanelItems(overrides, { planName: '夏季作息', date: '2026-04-26' })

    expect(groups).toHaveLength(2)
    expect(groups.map((group) => group.overrideId)).toEqual(['override-a', 'override-b'])
    expect(groups.every((group) => group.canUndo)).toBe(true)
