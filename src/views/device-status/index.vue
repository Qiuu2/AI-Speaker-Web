            ...
            <div class="device-actions">
              <div class="volume-control">
                ...
                <el-button
                  size="mini"
                  type="primary"
                  :loading="isSettingVolume(device)"
                  @click="applyVolume(device)"
                >
                  设置
                </el-button>
                <el-button
...
export default {
  ...
  methods: {
    ...
    handleTreeDrop(data, event) {
      ...
      this.dragTargetZoneId = ''
      this.clearDragExpandTimer()
      this.moveDialog.pending = {
        deviceId: dragging.id,
        deviceName: dragging.name,
        sourceZoneId: String(dragging.zoneId || 'unassigned'),
        targetZoneId,
        targetZoneName: this.zoneLabelById(targetZoneId)
      }
    ...
    async handleRemoveDropDrop(event) {
      ...
      this.syncRemoveDropFromDragging(dragging, 'loading')
      try {
        await moveTerminalZone({
          deviceId: dragging.id,
          sourceZoneId: String(dragging.zoneId || ''),
          targetZoneId: 'unassigned'
        })
        this.$message.success(`已将“${dragging.name}”移出 ${this.zoneLabelById(dragging.zoneId)}`)
        await this.refreshData(true)
      } catch (err) {
        const detail = err?.response?.data?.detail
        this.$message.error(detail || '移出失败，请重试')
      } finally {
        this.endDeviceDrag()
      }
    },
    ...
    async confirmMoveDialog() {
      ...
      const targetZoneId = String(pending.targetZoneId || '')
      try {
        await moveTerminalZone({
          deviceId: pending.deviceId,
          targetZoneId,
          sourceZoneId: pending.sourceZoneId
        })
        ...
        const detail = err?.response?.data?.detail
        this.$message.error(detail || '移动失败，请重试')
      } finally {
    ...
    openMoveDialogForDevice(device) {
      ...
      const sourceZoneId = this.preferredDeviceZoneId(device) || 'unassigned'
      this.moveDialog.pending = {
        deviceId,
        deviceName: String(device?.name || deviceId || '终端'),
        sourceZoneId,
        sourceZoneName: this.zoneLabelById(sourceZoneId),
        targetZoneId: '',
        targetZoneName: '',
        mode: 'manual'
      }
    ...
    async removeDeviceFromCurrentZone(device) {
      ...
      try {
        await this.$confirm(
          `确定将“${device.name || '终端'}”移出 ${this.zoneLabelById(sourceZoneId)} 吗？`,
          '确认移出分区',
          {
            confirmButtonText: '移出分区',
            cancelButtonText: '取消',
            type: 'warning'
          }
        )
    ...
    isSettingVolume(device) {
      const key = String(this.getTerminalId(device))
      return Boolean(this.settingVolume[key])
    },
    async setVolume(device, volume, silent = false) {
      const deviceId = this.getTerminalId(device)
      if (!deviceId) {
        if (!silent) {
          this.$message.warning('缺少终端 ID，无法设置音量')
        }
        return
      }
      const safeVolume = Math.max(0, Math.min(100, this.toNumber(volume, 0)))
      device.volume = safeVolume
      const key = String(deviceId)
      const payload = {
        DEVICE_IP: device.ip || '',
        DEVICE_ID: String(deviceId),
        VOLUME: String(safeVolume),
        SYNC_TASKS: 0
      }
      this.$set(this.settingVolume, key, true)
      try {
        await setTerminalVolume(payload)
        this.syncDeviceVolumeMeta(deviceId, safeVolume)
        if (!silent) {
          this.$message.success(`已设置 ${device.name} 音量为 ${safeVolume}`)
        }
      } catch (err) {
        if (!silent) {
          const detail = err?.response?.data?.detail
          this.$message.error(detail || '音量设置失败')
        }
      } finally {
        this.$delete(this.settingVolume, key)
      }
    },
    applyVolume(device) {
      this.setVolume(device, device.volume)
    },
    ...
    toggleMute(device) {
      if (!device) return
      if (device.muted) {
        const restore = device.prevVolume || 30
        this.setVolume(device, restore, true)
      } else {
        this.setVolume(device, 0, true)
      }
    }
  }
}
