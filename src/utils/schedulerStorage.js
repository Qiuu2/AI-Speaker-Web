...
// T51 ③: canonicalize PURELY DERIVED / format-only fields before a dirty diff,
// so a locally-built row and the server's canonical form of the same task are
// not flagged dirty over cosmetic differences. This must NOT touch any
// user-editable field — in particular `status` is left untouched, because
// setBroadcastStatus batch staging legitimately lights the draft via a status
// difference (KP #22 r2). Only the redundant/derived trio is normalized:
//   - taskstate: derived from status (redundant); drop it from the compare so
//     null vs 0 across the local/server fork does not register.
//   - time / starttime: align to the backend's HH:MM:SS form so 'HH:MM' and
//     'HH:MM:00' compare equal — but a REAL second-level difference is kept.
//   - liveterminalname: a derived display label; unify empty/undefined.
//
// time/starttime alignment mirrors backend format_hhmmss (helpers.py:36):
// 2-segment 'HH:MM' is padded to 'HH:MM:00', 3-segment 'HH:MM:SS' kept as-is.
// This only removes the digit-count equivalence ('08:00' vs '08:00:00'); it does
// NOT strip seconds, so '08:00:30' stays distinct from '08:00:00' (a genuine
// second-level edit still registers as dirty — the broadcast time picker accepts
// seconds and the vendor stores them, per backend format_hhmmss).
function padToHourMinuteSecond(value) {
  const text = String(value == null ? '' : value).trim()
  const match = text.match(/^(\d{1,2}:\d{2})(:\d{2})?$/)
  if (!match) return text
  return match[2] ? text : `${match[1]}:00`
}

function canonicalizeBroadcastRowForCompare(row) {
  if (!row || typeof row !== 'object') return row
  const next = { ...row }
  // taskstate is derived from status (which is compared) — drop it.
  delete next.taskstate
  // liveterminalname is a derived display label the server may blank out even
  // when the local row carried one; the binding is carried by liveterminalid /
  // terminalids, so drop the label from the compare (do NOT just coerce — a
  // populated-vs-empty pair would still differ).
  delete next.liveterminalname
  // time / starttime: align 'HH:MM' to 'HH:MM:00' (backend format_hhmmss), keep
  // a real second-level difference.
  if ('time' in next) next.time = padToHourMinuteSecond(next.time)
  if ('starttime' in next) next.starttime = padToHourMinuteSecond(next.starttime)
  return next
}

function normalizeBroadcastSnapshotForCompare(value) {
  return normalizeBroadcastSnapshot(value).map((row) => canonicalizeBroadcastRowForCompare(row))
}

function areBroadcastSnapshotsEqual(left, right) {
  return JSON.stringify(normalizeBroadcastSnapshotForCompare(left)) === JSON.stringify(normalizeBroadcastSnapshotForCompare(right))
}

// T51 ②: exported for the loadBroadcasts hasDraft branch — two broadcast row
// lists are "substantively equal" when they match after derived/format-only
// canonicalization. The hasDraft branch uses this to auto-clear a draft whose
// only remaining difference from the server is a derived field, instead of
// re-saving it forever (the self-perpetuating draft bug). Substantive user
// edits (status / volume / audio / terminals) still register as a difference
// and keep the draft (KP #22 r2).
export function areBroadcastRowsSubstantivelyEqual(left, right) {
  return areBroadcastSnapshotsEqual(left, right)
}
...
function calculateBroadcastDraftMeta(broadcasts, broadcastBaseSnapshot) {
  ...
  currentMap.forEach((row, rowId) => {
    ...
    // T51 ③: compare on the derived-field-canonical form so a local row only
    // differing from the server canonical form in taskstate / time format /
    // liveterminalname is NOT flagged dirty. Substantive fields (status,
    // volume, audio, terminals …) are compared as-is.
    if (!baseRow ||
      JSON.stringify(canonicalizeBroadcastRowForCompare(row)) !== JSON.stringify(canonicalizeBroadcastRowForCompare(baseRow))) {
